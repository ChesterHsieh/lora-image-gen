...
> ⚠️ **蒐集這步請自行處理**：大量爬圖有版權 / ToS 風險，請手動把卡牌圖存到 `raw_cards/`。
> 版權注意事項見 [../runpod/DEPLOY.md](../runpod/DEPLOY.md)：個人學習 / 研究多半 OK，
> 公開分享或商用前請先確認來源素材的授權。
...
先把檔名與描述整理成 `cards.csv`（表頭 `filename,description`，見 `cards.csv.example`）。
`filename` 對應 `cropped/` 裡的圖檔名（可含或不含副檔名）。

```bash
# dry-run：先看 caption 會長怎樣、哪些風格詞被剝掉、哪些圖缺描述
python make_captions.py --images cropped --csv cards.csv --trigger mystyle --dry-run
...
**它幫你守住兩條風格 LoRA 鐵律：**
- 每張 caption 開頭加觸發詞（預設 `mystyle`，用模型不認識的字召喚風格）。
